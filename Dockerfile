...
#### generate build --prod
RUN ng build --configuration=production --output-hashing=none && ng run globex-partner:server:production --output-hashing=none
...
RUN pwd
RUN ls -a
CMD node ./dist/globex-partner/server/main.js
