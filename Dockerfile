...
#### generate build --prod
RUN ng build --configuration=production --output-hashing=none && ng run globex-partner-web:server:production --output-hashing=none
...
RUN pwd
RUN ls -a
CMD node ./dist/globex-partner-web/server/main.js
